...
from common.my_formatter import qiskit_to_my_format_circuit, layered_instructions_to_circuit, get_layered_instructions
...
def get_layer_type(layer2instructions, n_qubits):
    num_layer = len(layer2instructions)
    layer_type = [0 for _ in range(num_layer)]
    layer_info = [[None for _ in range(num_layer)] for _ in range(n_qubits)]
    for index, layer in enumerate(layer2instructions):
        layer_type[index] = len(layer[0].qubits)
        for ins in layer:
            gate_name = ins.operation.name
            for qubit_index, qubit in enumerate(ins.qubits):
                if gate_name == 'rx' or gate_name == 'ry':
                    layer_info[qubit.index][index] = [gate_name, round(float(ins.operation.params[0]) / np.pi, 4)]
                elif gate_name == 'id' or gate_name == 'rz':
                    continue
                elif gate_name == 'cz':
                    layer_info[qubit.index][index] = [gate_name, ins.qubits[1 - qubit_index].index]
                else:
                    continue
